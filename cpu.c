...
#define SIZE_CALLBACK_1 (sizeof(callback_1) / sizeof(callback_1[0]))
    ...
    {
        const uint16_t arg_1 = ((uint16_t*)(instr + 1))[0];
        callback_1[opcode - SIZE_CALLBACK_0 - 1](arg_1);
    }
    ...
    else if(opcode == OPCD_POP)
    {
        const uint16_t arg_1 = ((uint16_t*)(instr + 1))[0];
        _pop(&reg[arg_1]);
    }
    else if(opcode == OPCD_ABORT)
    {
        _abort();
    }
    else if(opcode == OPCD_NOT)
    {
        _not();
    }
    else if(opcode == OPCD_RET)
    {
        _ret();
    }
}
...
void selection()
{
    static const uint8_t instruction_size[] = 
    {
        ...
        SIZE_XOR, SIZE_CMP, SIZE_BNOT,SIZE_JMP,
        SIZE_JZ , SIZE_JNZ, SIZE_JS,  SIZE_JNS,
        SIZE_JL, SIZE_JNL, SIZE_CALL,SIZE_PUSH,
        SIZE_INT, SIZE_POP, SIZE_NOT, SIZE_RET,
        SIZE_ABORT
    };
    ip = ram[0];/*start instruction segment*/
...
void load_to_memory(char* filename)
{
    ...
    fread(ram, sizeof(uint8_t), flen, fptr);
    fclose(fptr);
}
